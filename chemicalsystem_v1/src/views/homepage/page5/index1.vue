    ...
    <div>
        ...
        <span>
          ...
          <el-input v-model="search" placeholder="请输入要搜索的内容" size="small" class="elinput1"></el-input>
                    <!--          <el-button size="small" type="primary" class="elbtnsearch1"-->
                    <!--          >搜索</el-button-->
                    <!--          >-->
        </span>
            </el-row>
        ...
        <div>
            <el-table
                    class="eltable1"
                    v-loading="loading"
                    element-loading-text="拼命加载中(ಥ_ಥ)"
                    :data="handleSearch"
            ...
            >
              ...
              scope.row.export_time.substr(0, 16)
                ...
                <el-table-column prop="medicine_box" label="药品柜"></el-table-column>
                <el-table-column
                        prop="expired_time"
                        :show-overflow-tooltip="true"
                        label="过期日期"
                >
                    <template slot-scope="scope">
                        <i class="el-icon-date"></i>
                        <span style="margin-left: 0.5vw">{{
              scope.row.expired_time.substr(0, 10)
            }}</span>
                    </template>
                </el-table-column>
    ...
    export default {
        ...
        created() {
            ...
            axios
                .get(
                    api123.baseURL +
                    "/Exportchemicalmedicine/findexportmedicineandsortPageable/1/10/ascending/id"
                )
                .then((response) => {
        ...
        methods: {
            ...
            handleSizeChange(val) {
                ...
                axios
                    .get(
                        api123.baseURL +
                        "/Exportchemicalmedicine/findexportmedicineandsortPageable/" +
            ...
            handleDelete(row) {
                ...
                this.$confirm("此操作将永久删除该留言且不可恢复, 是否继续?", "提示", {
                    ...
                    .then(() => {
                        axios
                            .get(api123.baseURL + "/Exportchemicalmedicine/lover/" + row.id)
                            .then(function (resp) {
                            });
            ...
            // 添加新药跳转
            addNewMedicine() {
                this.$router.push("/page5exportdate1");
            },
            // 下载excel模板
            downloadExcel() {
                window.location.href = api123.baseURL + "/chemicalfiles/5DF9994C97796D4AB9C91C11530A8413.xlsx";
                // ('http://192.168.31.100:9528/chemicalfiles/AViR3FlrXtY3wT7LFqQsODzWSkW38JxwcLcMVGelI4.wG0ktrvKa.jpg');
                // axios
                // .get(api123.baseURL + "/chemicalfiles/5DF9994C97796D4AB9C91C11530A8413.xlsx")
            },
        },
    };
</script>
